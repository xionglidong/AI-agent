...
class PerformanceAnalyzer {
    ...
    checkAlgorithmicComplexity(code, _language) {
        const issues = [];
    ...
    checkMemoryUsage(code, _language) {
        const issues = [];
        const lines = code.split('\\n');
        lines.forEach((line, index) => {
            const lineNumber = index + 1;
    ...
    checkAsyncPatterns(code, _language) {
        const issues = [];
        const lines = code.split('\\n');
        lines.forEach((line, index) => {
            const lineNumber = index + 1;
            ...
            if (trimmedLine.includes('async function') || trimmedLine.includes('async ')) {
                ...
                const hasPromises = functionLines.some(l => l.includes('.then(') ||
                    l.includes('Promise.') ||
                    l.includes('fetch(') ||
    ...
    checkDOMOperations(code, language) {
        const issues = [];
    ...
    checkDatabaseQueries(code, _language) {
        const issues = [];
        const lines = code.split('\\n');
        lines.forEach((line, index) => {
            const lineNumber = index + 1;
